...
Reproduces Figure 4 from the paper: sweeps polynomial basis dimension K
and shows the three-regime phase transition (underfitting, selective overload,
...
def main():
    ...
    print(f"N={N}, sel={sel}, {n_seeds} seeds")
    print()
    print(f"{'Degree':>6s} {'K':>5s} {'RMSE_std':>10s} {'RMSE_SA':>10s} {'Improv%':>8s}")
    print("-" * 45)
    ...
    for deg in degrees:
        ...
        rmse_std = math.sqrt(sum(b ** 2 for b in biases_std) / len(biases_std))
        rmse_sa = math.sqrt(sum(b ** 2 for b in biases_sa) / len(biases_sa))
        improv = (rmse_std - rmse_sa) / rmse_std * 100
        print(f"{deg:>6d} {K:>5d} {rmse_std:>10.4f} {rmse_sa:>10.4f} {improv:>7.1f}%")

    print()
    print("Expected: gain near 0 at degree 1, positive at degree 2-3, collapse at degree 4-5")
