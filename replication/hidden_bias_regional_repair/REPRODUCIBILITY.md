...
Paper-facing figures must be regenerated by checked-in source code from the
canonical accepted data artifacts. Do not crop, slice, stitch, mask, relabel,
or rearrange panels from rendered PNG, PDF, SVG, screenshot, or other finished
figures. This prohibition includes TeX `trim`, `clip`, and `viewport`
composition and pixel-level editing. Do not reconstruct a plot from values
copied from manuscript prose or tables.

Implement layout changes in the original generator or editable vector source
and regenerate the complete figure. If that source is unavailable, leave the
figure unchanged until it is recovered or replaced by a reproducible
source-level drawing. Preserve vector output for plots and diagrams that
originate as vectors. TeX may uniformly scale and place a complete figure; it
may not act as an image editor. Record the generator, canonical input,
regeneration command, and output path for every changed paper-facing figure.
