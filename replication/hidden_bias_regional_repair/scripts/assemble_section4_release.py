...
def main() -> None:
    ...
    # Emit complete table environments.  The manuscript inputs these files
    # verbatim; it does not hand-enter numerical cells or wrap generated row
    # fragments in a separately maintained booktabs environment.
    natural_specs = (
        ("Ma DGP~2", "DR-BC ATT", "ma_dgp2"),
        ("Ma DGP~3", "DR-BC ATT", "ma_dgp3"),
        ("Ma family", "DR-BC ATT", "ma_family"),
        ("Kang--Schafer", "C-TMLE", "ks_ctmle"),
    ...
    def table_tex(
        ...
        *,
        ...
        lines = [
            ...
            f"\\caption{{{caption}}}\n",
            f"\\label{{{label}}}\n",
            f"\\begin{{tabular}}{{@{{}}p{{{first_width}\\linewidth}}p{{.21\\linewidth}}rp{{.27\\linewidth}}@{{}}}}\n",
            "\\toprule\n",
            "design & upstream estimator & cells & MSE gain (95\\% CI) \\\\\n",
            "\\midrule\n",
        ]
        value_keys = {str(row["key"]) for row in values}
        for design, method, key in specs:
            if key not in value_keys:
                raise SystemExit(f"generated table references missing key: {key}")
            stem = macro_stem(key)
            lines.append(
                f"{design} & {method} & \\{stem}Cells & "
                f"$\\{stem}Gain\\%\\ [\\{stem}Lo,\\{stem}Hi]$ \\\\\n"
            )
        lines.extend(["\\bottomrule\n", "\\end{tabular}\n", "\\end{table}\n"])
        return "".join(lines)

    (args.out_dir / "section4_natural_table.tex").write_text(
        table_tex(
            natural_specs,
            caption=(
                "End-to-end performance in natural settings at the frozen $c=2$. "
                "Gain is paired relative MSE reduction. Family rows weight native "
                "cells equally; brackets are paired 95\\% percentile-bootstrap intervals."
            ),
            label="tab:natural-end-to-end",
            first_width=".29",
        )
    )
    (args.out_dir / "section4_emphasized_table.tex").write_text(
        table_tex(
            emphasized_specs,
            caption=(
                "End-to-end performance in emphasized settings at $c=2$. Signal "
                "averages exclude the separately reported $s=0$ null. Exact zeros "
                "denote complete stand-down."
            ),
    ...
    verification = {
        ...
        "outputs": {
            ...
            for name in (
                ...
                "section4_values.tex",
                "section4_sensitivity_rows.tex", "section4_natural_table.tex",
                "section4_emphasized_table.tex",
                "README.md",
            )
        },
    }
