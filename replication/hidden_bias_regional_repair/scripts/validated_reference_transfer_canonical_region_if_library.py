...
def _high_score_region(score, response, region_quantile, min_observed=0):
    if min_observed > 0:
        order = np.argsort(-score, kind="mergesort")
        min_count = min(
            len(score),
        ...
        mask[order[:count]] = True
        return mask
    cutoff = float(np.quantile(score, 1.0 - region_quantile))
    return score >= cutoff
...
def _matched_high_score_region(score, response, region_quantile, min_observed):
    lowp = _low_score_region(score, response, region_quantile, min_observed)
    count = int(np.sum(lowp))
    if count <= 0:
        return np.zeros(len(score), dtype=bool)
    order = np.argsort(-score, kind="mergesort")
    mask = np.zeros(len(score), dtype=bool)
    mask[order[: min(count, len(order))]] = True
    return mask
...
def _estimated_residual_highp_region(
    x,
    y,
    response,
    true_pi,
    ...
    highp = (
        _matched_high_score_region(
            response_score, response, region_quantile, min_observed
        )
        if match_lowp_size
        else _high_score_region(
            response_score, response, region_quantile, min_observed
        )
    )
    observed = response.astype(bool)
    observed_highp = observed & highp
    empty = np.zeros(len(y), dtype=bool)
    if observed_highp.sum() < max(3, min_observed):
        return empty if selector_ablation in standdown_ablations else highp
    ...
    direction = float(np.mean(weighted_residual[observed_highp]))
    if direction == 0.0 or not np.isfinite(direction):
        return empty if selector_ablation in standdown_ablations else highp
    ...
    inside = np.flatnonzero(highp)
    if len(inside) == 0:
        return empty if selector_ablation in standdown_ablations else highp
    best_mask = empty if selector_ablation in standdown_ablations else highp
    best_score = 0.0
    for rank_sign in rank_signs:
        order = inside[np.argsort(-(rank_sign * rank_signal[inside]), kind="mergesort")]
        if selector_ablation == "all_prefixes":
        ...
        for count in candidate_counts:
            mask = np.zeros(len(highp), dtype=bool)
            mask[order[:count]] = True
            if int(np.sum(mask & observed)) < max(3, min_observed):
                continue
            score_sign = (
...
def _analysis_region(
    ...
    if variant in {
        ...
        return _estimated_residual_highp_region(
            x,
            y,
            response,
            true_pi,
            ...
            folds,
            match_lowp_size=(
                variant == "estimated_residual_highp_matched_lowp_supported"
            ),
        )
...
def _crossfit_selected(
    ...
    ma_dr_bc_stats_by_tau = {}
    ma_projection_scores = {}
    ...
    if reference_method == "ma_dr_bc":
        ...
        m_oof = initial_outcome_values
        for trim_h in tau_grid:
            dr_bc_values, ma_dr_bc_stats_by_tau[trim_h] = _ma_dr_bc_reference(
                y,
                response,
                p_raw,
                m_oof,
            ...
            ma_projection_scores[trim_h] = _crossfit_ma_dr_bc_score(
                y,
                response,
                p_raw,
                m_oof,
    ...
    if repair_mode in {"if_projection", "regional_if_projection"}:
        ...
        for tau in projection_taus:
            ...
            selection_score = (
                np.asarray(ma_projection_scores[tau], dtype=float)
                if reference_method == "ma_dr_bc"
                else base_score
            )
    ...
    if repair_mode in {"if_residual", "regional_if_residual"}:
        ...
        for tau in residual_taus:
            ...
            # The two residual rules are identical except for this support
            # mask.  Both learn the same direction, use the same score-risk
            # gate, and traverse the same damping path.
    ...
    if repair_mode == "if_library":
        ...
        for tau in residual_taus:
            ...
            for region_damp in region_damp_grid:
                ...
                candidate_outcome = (
                    base_outcome + region_damp * correction * region.astype(float)
                )
                score_contrast = (
                ...
                if regional_is_eligible:
                    ...
                    candidate_kind[(tau, region_damp)] = (
                        "reference" if region_damp == 0.0 else "regional_if_residual"
                    )
                damp_losses[(tau, region_damp)] = candidate_loss.tolist()
                damp_improvements[(tau, region_damp)] = (
                    baseline_loss - candidate_loss
    ...
    if reference_method == "ma_dr_bc":
        ma_dr_bc_stats = ma_dr_bc_stats_by_tau[selected]
    mean_damp_losses = {
        g: (
    ...
    return {
        "ref": returned_ref,
        "rt": returned_rt,
        # Store the complete path so alternative gates can be recomputed
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default="obsval",
        help=(
            "Rule for choosing the regional damping value: held-out validation "
